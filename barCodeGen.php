...
foreach ($eans as $x){
    echo $x; // do something with each line from text file here




$bar= new barcode();
$barnumber=substr($x,8 ,13);
$bar->_encode   = 'EAN-13';
$bar->_color    = '#000000';
$bar->_bgcolor  = '#FFFFFF';
$bar->_height   = 50;
$bar->_font     = '/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf';
$bar->_scale    = 2;
$bar->_format   = 'gif';
$filename = substr($x,0 ,8);
try
   {
    $bar->genBarCode( $barnumber, 'gif', $filename);
   }
catch( Exception $e )
   {
    echo $e->getMessage()."\n";
